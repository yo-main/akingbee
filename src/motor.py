...
def get_all(class_, deepth=0):
    return factory.get_from_filters(
        class_, {"user": flask.session["user_id"]}, deepth
    )
...
def registerCheck():
    ...
    user.pwd = generate_password_hash(
        data["pwd"], method="pbkdf2:sha256", salt_length=8
    )
    user.save(facto)
...
def submit_beehouse_details():
    beehouse = factory.get_from_id(
        flask.request.form.get("bh_id"), objects.Beehouse
    )
...
def beehouse_profil():
    ...
    return render(
        "akingbee/beehouse/beehouse_details.html",
        beehouse=beehouse,
        comments=comments,
        actions=actions,
...
def solve_action():
    ...
    action = factory.get_from_id(
        flask.request.form.get("ac_id"), objects.Actions
    )
...
@login_required
def edit_comment():
    comment = factory.get_from_id(
        flask.request.form.get("cm_id"), objects.Comments
    )
...
def setupStatusBh():
    ...
    if lang == config.FRENCH:
        ...
        description = (
            "Les différents status qui peuvent " "être affectés à une ruche"
        )
    else:
        title = "Beehouse status"
        description = "The different status that can " "be given to a beehouse"
...
def setupHealth():
    ...
    if lang == config.FRENCH:
        ...
        description = (
            "Les différents status de santés "
            "que vous souhaitez affecter à une ruche"
        )
    else:
        title = "Health status"
        description = (
            "Different health status that " "you wish to affect to a beehouse"
        )
...
def setupBh_actions():
    ...
    if lang == config.FRENCH:
        ...
        description = (
            "The différent actions that " "you may have to do on a beehouse"
        )
