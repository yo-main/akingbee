...
def test_delete_swarm(auth_token, test_app):
    ...
    response = test_app.post("/swarm", json=swarm_body, cookies={"access_token": auth_token})
    assert response.status_code == 200

    swarm_id = response.json()["id"]

    hive_body = {"name": "test", "condition_id": str(IDS["Hive_conditions"][0]), "owner_id": str(IDS["Owners"][0]), "swarm_id": swarm_id}
    response = test_app.post("/hive", json=hive_body, cookies={"access_token": auth_token})
    assert response.status_code == 200

    hive_with_swarm = response.json()
    assert str(hive_with_swarm["swarm"]["id"]) == swarm_id

    response = test_app.delete(f"/swarm/{str(swarm_id)}", cookies={"access_token": auth_token})
    assert response.status_code == 204

    response = test_app.get(f"/hive/{str(hive_with_swarm['id'])}", cookies={"access_token": auth_token})
    assert response.status_code == 200
