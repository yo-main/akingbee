...
async def post_swarm(input: PostSwarmIn, user: User = Depends(auth_user)):
...
async def get_swarm(swarm_id: UUID, user: User = Depends(auth_user)):
    swarm_entity = await SwarmQuery().get_swarm(swarm_id)
    return swarm_entity.asdict()


@router.put("/{swarm_id}", response_model=SwarmOut)
async def put_swarm(swarm_id: UUID, input: PutSwarmIn, user: User = Depends(auth_user)):
