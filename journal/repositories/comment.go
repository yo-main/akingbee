...
const queryListApiariesWithComment = `
	WITH MOST_RECENT_COMMENT AS (
		...
		FROM COMMENT
		JOIN HIVE ON HIVE.ID=COMMENT.HIVE_ID
		GROUP BY HIVE.APIARY_ID
	)
	SELECT 
		APIARY.PUBLIC_ID AS APIARY_PUBLIC_ID,
		...
		COALESCE(date(LAST_VISIT.DATE), '') AS LAST_VISIT_DATE
	FROM APIARY
	...
	LEFT JOIN LAST_VISIT ON LAST_VISIT.APIARY_ID=APIARY.ID
