...
@pytest.mark.parametrize(
    "name, location, honey_id, expected",
    (
...
def test_post_apiary_and_delete(auth_token, test_app):
    ...
    response = test_app.delete(
        f"/apiary/{obj_id}", cookies={"access_token": auth_token}
    )
...
def test_put_apiary(auth_token, test_app, obj_id, data, expected):
    ...
    response = test_app.put(
        f"/apiary/{str(obj_id)}", cookies={"access_token": auth_token}, json=data
    )
    assert response.status_code == expected
