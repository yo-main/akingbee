...
def test_rbmq_consumer():
    ...
    rbmq = RBMQConsumer(
        handlers=handlers, queue="test", connection_manager=conn_manager
    )

    # send messages
    rbmq.process(
        channel=mocked_channel, method=method("1", "key_1"), properties=None, body=None
    )
    rbmq.process(
        channel=mocked_channel, method=method("2", "key_2"), properties=None, body=None
    )
    rbmq.process(
        channel=mocked_channel,
        method=method("pouet", "pouet"),
        properties=None,
        body=None,
    )
    rbmq.process(
        channel=mocked_channel, method=method("3", "key_3"), properties=None, body=None
    )
    rbmq.process(
        channel=mocked_channel, method=method("4", "key_4"), properties=None, body=None
    )
    rbmq.process(
        channel=mocked_channel,
        method=method("boom", "boom"),
        properties=None,
        body=None,
    )
    rbmq.process(
    ...
    assert mocked_channel.basic_ack.mock_calls == [
        call("1"),
        call("2"),
        call("4"),
        call("4"),
