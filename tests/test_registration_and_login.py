...
@pytest.mark.parametrize("password", [
    ...
    "AZEbazeabAZE",
    "AZEAZ123123",
]) def test_register_user_wrong_password(password, client): form = {
        "username": "Yo",
...
def test_user_login_wrong_password(client):
    ...
    assert answer.status_code == 500
    assert json["code"] == alert_codes.INCORRECT_PASSWORD_ERROR
...
def test_login_unknown_username(client):
    ...
    json = answer.get_json()
    assert answer.status_code == 500
    assert json["code"] == alert_codes.USER_NOT_FOUND_ERROR
    assert json["status"] == "error"
...
def test_user_login_success(client):
    ...
    json = answer.get_json()
    assert answer.status_code == 200
    assert json["code"] == alert_codes.LOGIN_SUCCESS
